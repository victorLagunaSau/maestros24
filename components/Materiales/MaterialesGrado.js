...
import materiales from "../../graphql/materiales";

const MaterialesGrados = ({ grado }) => {
    const { loading, error, data } = useQuery(materiales, {
    ...
    const filteredResources = data.resources.filter(
        (resource) => resource.type !== "PATHBOOK"
    );
...
export default MaterialesGrados;
